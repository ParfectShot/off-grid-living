...
export const processServerImage = createServerFn({ method: "POST" })
  .validator(
    (data: {
      ...
      if (!data.inputPath || !data.outputDir || !data.outputFilename) {
        throw new Error(
          "Input path, output directory and filename are required"
        );
      }
      return data;
    }
  ...
  .handler(async ({ data }) => {
    ...
    if (!fs.existsSync(fullInputPath)) {
      console.error(
        `Input file not found at path: ${fullInputPath}. CWD: ${process.cwd()}`
      );
    ...
    const outputBuffer = await Sharp(inputBuffer)
      .resize({ width: width || originalWidth, withoutEnlargement: true })
      .webp()
      .toBuffer();
    ...
    for (const filePath of filePaths) {
      try {
        ...
        if (
          ...
          console.warn(
            `Skipping deletion of potentially unsafe path: ${filePath}`
          );
          errors.push({
            path: filePath,
            error: "Attempted to delete outside allowed directory",
          });
          continue;
        }
